...
public class IBmsTagServiceImpl extends ServiceImpl<BmsTagMapper, BmsTag> implements IBmsTagService {
    ...
    @Override
    public Page<BmsTopic> selectTopicsByTagId(Page<BmsTopic> topicPage, String id) {
        // 获取关联的话题ID
        Set<String> ids = iBmsTopicTagService.selectTopicIdsByTagId(id);
        LambdaQueryWrapper<BmsTopic> wrapper = new LambdaQueryWrapper<>();
        wrapper.in(BmsTopic::getId, ids);

        return iBmsTopicService.page(topicPage, wrapper);
    }
